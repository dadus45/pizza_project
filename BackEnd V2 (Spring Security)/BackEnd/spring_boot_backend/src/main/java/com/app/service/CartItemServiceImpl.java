...
//		cartItemsDto.forEach((item)->item.setUserName(userId));
	...
	@Override
	public List<CartItemChartDto> getChartCartItems() {
		ResultTransformer transformer = Transformers.aliasToBean(CartItemChartDto.class);
		
		return cartItemRepo.findChartData(transformer);
	}
